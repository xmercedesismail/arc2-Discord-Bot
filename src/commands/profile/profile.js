...
module.exports = async (client, interaction, args) => {
    ...
    if (data) {
        ...
        client.embed({
            ...
            {
                name: "🐶┆Favorite pets",
                value: `${data.Pets && data.Pets.length > 0 ? data.Pets.join(', ') : 'Not set'}`,
                inline: true
            },
            {
                name: "🍕┆Favorite food",
                value: `${data.Food && data.Food.length > 0 ? data.Food.join(', ') : 'Not set'}`,
                inline: true
            },
            {
                name: "🎶┆Favorite songs",
                value: `${data.Songs && data.Songs.length > 0 ? data.Songs.join(', ') : 'Not set'}`,
                inline: true
            },
            {
                name: "🎤┆Favorite artists",
                value: `${data.Artists && data.Artists.length > 0 ? data.Artists.join(', ') : 'Not set'}`,
                inline: true
            },
            {
                name: "🎬┆Favorite movies",
                value: `${data.Movies && data.Movies.length > 0 ? data.Movies.join(', ') : 'Not set'}`,
                inline: true
            },
            {
                name: "👨‍🎤┆Favorite actors",
                value: `${data.Actors && data.Actors.length > 0 ? data.Actors.join(', ') : 'Not set'}`,
                inline: true
            },
            {
                name: "🏴┆Origin",
                value: `${data.Origin || 'Not set'}`,
                inline: true
            },
            {
                name: "🎮┆Hobby's",
                value: `${data.Hobbys && data.Hobbys.length > 0 ? data.Hobbys.join(', ') : 'Not set'}`,
                inline: true
            },
            {
                name: "😛┆Status",
